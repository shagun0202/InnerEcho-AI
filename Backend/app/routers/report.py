...
from app.services.wellness_service import get_profile
...
def get_weekly_report(
    ...
    try:
        if gemini.client is None or not get_profile(db, current_user.id).ai_consent:
        ...
        if "---NARRATIVE---" in text:
            ...
            for i, part in enumerate(parts):
                if "NARRATIVE" in part and i + 1 < len(parts):
                    ...
                    raw = parts[i + 1].strip()
                    highlights = [
                        h.strip().lstrip("•-* ") for h in raw.split("\n") if h.strip()
                    ]
